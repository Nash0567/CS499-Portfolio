...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    string pivot = bids[(begin + end) / 2].title; // Middle element as pivot
...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    int mid = partition(bids, begin, end); // partition the vector

    // recursively sort low partition (begin to mid)
    if (begin < mid) quickSort(bids, begin, mid);

    // recursively sort high partition (mid+1 to end)
    if (mid + 1 < end) quickSort(bids, mid + 1, end);
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 4:  // QuickSort
            ...
            ticks = clock() - ticks;
            cout << "QuickSort completed in " << (ticks * 1.0 / CLOCKS_PER_SEC) << " seconds" << endl;
            break;
